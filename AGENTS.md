...
- The final choice is `argmax` over deterministic candidate utility. Gemini, Moss, and Bright Data never select the winner.
...
- Next.js App Router, strict TypeScript, React, Tailwind CSS, Vitest, Zod, Gemini API, `@moss-dev/moss`, lucide-react, and browser-native speech, geolocation, and Wake Lock APIs.
...
- Gemini may interpret language, but it must never rank candidates or generate numeric scores.
