      ...
      {snapshot.rankedCandidates.slice(0, 3).map((candidate) => {
        ...
        return (
          <div key={candidate.candidateId}>
            ...
            {after && delta && <small>{delta.scoreDelta >= 0 ? "+" : ""}{delta.scoreDelta.toFixed(1)} · {delta.primaryCauses[0]}</small>}
          </div>
        );
      })}
  ...
  return (
    <section className="decision-shift">
      ...
      <div className="shift-grid">
        ...
        <div className="taught-card"><span>User taught the agent</span><blockquote>“{memory.text}”</blockquote><div><DatabaseZap size={14} /><strong>{mossStatus === "Live" ? "Stored in Moss" : "Moss fallback memory"}</strong></div><small>Topic: {memory.topic} · Polarity: negative · Strength: {memory.strength}</small></div>
        <ArrowRight className="shift-arrow" size={18} />
